...
const JT_JA: Record<string, string> = {
  ...
  chandraTitle: 'なぜチャンドラ・ラグナ（月のアセンダント）なのか？',
  chandra:
    'インド占星術において、月（チャンドラ）は心・感情・日々の実際の経験を映します。月の星座を第1ハウス（チャンドラ・ラグナ）として — 出生ラグナと併せて — 鑑定することで、その方の内面と出来事のタイミングを最も正確かつ実践的に描き出すことができます。当ポータルでは、その両方を重視して読み解きます。',
  instrTitle: '正確な鑑定のために、以下をご確認ください',
  instr1: '出生時刻は正確である必要があります — 数分の違いでもラグナが変わります。午前・午後（AM / PM）を慎重にご確認ください。',
  instr2: '（記録どおりの）正確な生年月日と、正しい座標・タイムゾーンを得るために出生地の市町村を正確にご入力ください。',
  instr3: '出生時刻が不確かな場合は、ラグナに基づく鑑定よりも、月（チャンドラ）に基づく鑑定の方が信頼できます。',
  tabReading: '鑑定',
